    ...
    return (
      <div className={styles.item} key={id}>
        ...
        <div className={styles.info}>
          <div className={styles.infoItem1}><h3 className={styles.title}>{title}</h3></div>
          <div className={styles.infoItem2}>
            ...
            <div className={styles.cantidadContainer}>
                ...
                <input className={styles.cantidadInput}/> {/* aca va quantity */}
            </div>
            <h2 className={styles.precio}>Total: $ {price}</h2>
            <button className={styles.itemCancelar} onClick={() => handleOnDelete(id, 0, 0)}><FaTrashAlt color="#01A86C" size="1rem" /></button>
          </div>
        </div>
      </div>
    );
          ...
          <div className={styles.containerGeneral}>
            ...
            <div className={styles.container}>
              ...
              <div className={styles.container1}>
                ...
                <Button className={styles.textoContinuarComprando} leftIcon={<FaBackward />} bg="none" _hover={{bg:'none'}}  _active={{bg:'none'}} _focus={{borderColor:'none'}}> 
                Continuar comprando...
                </Button>
                </div>

                {guestCartBooks?.length > 0 ? (<div>{item}</div>) : (
              ...
              {guestCartBooks?.length > 0 && <div className={styles.infoCompra}>
                  <div className={styles.total}>
                    <h3>Total a pagar: ${parseFloat(totalAmount).toFixed(2)}</h3>
                  </div>
                  <div className={styles.button}>
                    <button className={styles.comprar} onClick={(e) => handleBuyingBooks(e)}>Comprar</button>
                  </div>
                </div> 
              }
              </div>
